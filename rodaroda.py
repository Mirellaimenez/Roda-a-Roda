...
while acabou == False:
    ...
    rodar = input("Deseja rodar a roda? (Digite S para sim, qualquer coisa para não)").upper()
    if rodar == "S":
                ...
                if letra in palavra:
                    ...
                    for j in range(len(palavra)):
                        if palavra[j] == letra:
                            painel[i][j] = letra
                            letraAcertada +=1
                            ponto = letraAcertada * int(escolha_item)
            pontuacao[jogadores[indiceJogador]] += ponto
